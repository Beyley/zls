...
fn analyzeBodyInner(
    ...
    const result: Allocator.Error!Zir.Inst.Index = for (body) |inst| {
        const index: Index = switch (tags[inst]) {
            ...
            .field_type                   => .none,
            .field_type_ref               => .none,
            .ptr_to_int                   => try sema.getUnknownValue(Index.usize_type),
            ...
            .float_to_int                 => .none,
            .int_to_float                 => .none,
            .int_to_ptr                   => try sema.getUnknownValue(Index.usize_type),
            .float_cast                   => .none,
            .int_cast                     => .none,
            .ptr_cast                     => .none,
            .truncate                     => .none,
...
fn getUnknownValue(sema: *Sema, ty: Index) Allocator.Error!Index {
    std.debug.assert(sema.indexToKey(ty).typeOf() == .type_type);
    return try sema.get(.{ .unknown_value = .{ .ty = ty } });
}
