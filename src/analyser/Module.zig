...
pub fn relativeToNodeIndex(decl: Decl, offset: i32) Ast.Node.Index {
    return @bitCast(Ast.Node.Index, offset + @bitCast(i32, decl.node_idx));
}
...
pub fn allocateNewDecl(
    ...
    const decl_index = @intToEnum(DeclIndex, mod.ip.decls.len - 1);
...
pub fn declPtr(mod: *Module, decl_index: DeclIndex) *Decl {
    return mod.ip.getDecl(decl_index);
}
...
pub fn semaDecl(mod: *Module, decl_index: DeclIndex) Allocator.Error!void {
    ...
    if (mod.declIsRoot(decl_index)) {
        log.debug("semaDecl root {d} ({s})", .{ @enumToInt(decl_index), decl.name });
    ...
    log.debug("semaDecl {d} ({s})", .{ @enumToInt(decl_index), decl.name });
